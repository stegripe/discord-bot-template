...
export class CommandManager extends Collection<string, CommandComponent> {
                    ...
                    if (command.meta.slash && this.client.config.enableSlashCommand) {
                        ...
                        await this.registerCmd(command, {
                            onError: (
                                guild: Guild | null,
                                err: Error,
                                cmdType: "message" | "slash" | "user",
                            ) =>
                                this.client.logger.error(
                                    err,
                                    "Unable to register %s to %s command for %s",
                                    command.meta.name,
                                    cmdType,
                                    guild?.id ?? "???",
                                ),
                            onRegistered: (
                                ...
                                this.client.logger.info(
                                    "Registered %s to %s command for %s",
                                    command.meta.name,
                                    cmdType,
                                    guild?.id ?? "global",
                                ),
    ...
    private async registerDiscordCmd(
        cmd: BaseCommand,
        options: RegisterCmdOptions<true>,
    ): Promise<void> {
        ...
        if ((cmd.meta.contextChat?.length ?? 0) > 0) {
            await manager
                ...
                .catch((error: unknown) =>
                    options.onError(options.guild ?? null, error as Error, "message"),
                );
        }
        ...
        if ((cmd.meta.contextUser?.length ?? 0) > 0) {
            await manager
                ...
                .catch((error: unknown) =>
                    options.onError(options.guild ?? null, error as Error, "user"),
                );
        }

        if (cmd.meta.slash) {
            await manager
                ?.create(cmd.meta.slash as ApplicationCommandData)
                .catch((error: unknown) =>
                    options.onError(options.guild ?? null, error as Error, "slash"),
                );
        }
    }
